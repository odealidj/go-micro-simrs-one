...
const navGroups: NavGroup[] = [
  {
    title: null,
    items: [
      { name: "Dashboard", path: "/rekam-medis", exact: true, icon: LayoutDashboard },
    ],
  },
  {
    title: "Rekam Medis",
    items: [
      { name: "Cari Rekam Medis", path: "/rekam-medis/cari", icon: Search },
      { name: "Lihat Detail", path: "/rekam-medis/detail", icon: FileText },
    ],
  },
  {
    title: "Verifikasi ICD-10",
    items: [
      { name: "Pending Verifikasi", path: "/rekam-medis/icd10-pending", icon: ClipboardCheck },
    ],
  },
];

export function RekamMedisLayout() {
  ...
  const currentPageName =
    navGroups.flatMap((g) => g.items).find((item) => isActive(item.path, item.exact))?.name || "Rekam Medis";
          ...
          {!isSidebarOpen ? (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="leading-none">
                ...
                <span className="text-[10px] font-semibold text-indigo-600 tracking-wider uppercase">Unit Rekam Medis</span>
              </div>
            </div>
          )}
          <Button
              ...
              <nav className="space-y-1">
                {group.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <item.icon className={cn("h-5 w-5 shrink-0", active ? "text-indigo-600" : "text-slate-400 group-hover:text-slate-600")} />
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3 pl-2 pr-1 py-1 rounded-full border border-slate-200 bg-white shadow-sm">
              <div className="h-8 w-8 rounded-full bg-gradient-to-br from-indigo-500 to-indigo-400 flex items-center justify-center text-white font-bold text-sm">R</div>
              <div className="text-right hidden md:block">
                <p className="text-sm font-bold text-slate-800 leading-tight">Rekam Medis</p>
