...
export function AssignDokterPoliPage({ readOnly = false }: AssignDokterPoliPageProps) {
  ...
  const [doctorToUnassign, setDoctorToUnassign] = useState<any | null>(null);
  const [isUnassigning, setIsUnassigning] = useState(false);
  const [refreshKey, setRefreshKey] = useState(0);

  const { data: polyclinics } = useMasterData<any>("/master/polyclinics");

  // Ketika poliklinik dipilih di modal, cek hari mana saja yang sudah diisi dokter lain
  useEffect(() => {
    ...
    api
      .get(`/master/doctors/poli/${selectedPoli}`)
      .then((res) => {
        if (!isMounted) return;
        const docs = res.data?.data || [];
        const occ: Record<number, string> = {};
        docs.forEach((d: any) => {
          // Abaikan jadwal dokter yang sedang diedit agar hari miliknya sendiri tidak terblokir
          if (d.id !== selectedDoctor?.id && d.days_of_week && Array.isArray(d.days_of_week)) {
        ...
        setOccupiedDays(occ);
        // Otomatis buang hari yang sudah terisi dokter lain dari selectedDays
        setSelectedDays((prev) => {
          // Jika dokter baru di poliklinik ini atau belum ada hari, otomatis pilih semua hari yang tersedia (kosong)
          if (!selectedDoctor?.poli_code || selectedDoctor.poli_code !== selectedPoli) {
            const available = WORK_DAYS.filter((w) => !occ[w.id]).map((w) => w.id);
            return available;
          }
          // Jika dokter sedang mengedit jadwalnya di poli yang sama, pertahankan harinya dan buang yang bentrok dengan dokter lain
  ...
  }, [selectedPoli, isModalOpen, selectedDoctor]);
  ...
  const toggleDay = (dayId: number) => {
    if (occupiedDays[dayId]) return; // hari sudah diisi dokter lain
  ...
  const handleConfirmUnassign = async () => {
    ...
    try {
      await api.post("/master/doctors/unassign", {
        dokter_id: doctorToUnassign.id,
        poli_code: doctorToUnassign.poli_code,
      });
      toast.success(
        `Penugasan dr. ${doctorToUnassign.nama_lengkap || doctorToUnassign.username} berhasil dilepas.`
      );
      setDoctorToUnassign(null);
      setIsModalOpen(false);
      setRefreshKey((prev) => prev + 1);
    } catch (e: any) {
      toast.error(e.response?.data?.message || "Gagal melepas penugasan dokter.");
    } finally {
  ...
  const handleSave = async () => {
    if (!selectedPoli || !selectedDoctor || !startDate) return;
    if (selectedDays.length === 0) {
      setErrorMsg("Pilih minimal 1 hari kerja (Senin - Jumat) untuk dokter ini.");
      return;
    }

    // Validasi apakah ada hari terpilih yang bentrok
    ...
    setErrorMsg("");
    try {
      await api.post("/master/doctors/assign", {
      ...
      setIsModalOpen(false);
      toast.success("Penugasan dokter berhasil disimpan.");
      setRefreshKey((prev) => prev + 1);
    } catch (e: any) {
  ...
  return (
    <>
      ...
      <MasterDataTable<any>
        key={refreshKey}
        title="Assign Dokter"
        description="Pemetaan dokter ke poliklinik berbasis hari kerja (Senin – Jumat, 1 Shift 08:00 – 16:00)"
        endpoint="/master/doctors"
        requiresPoliFilter={true}
        columns={
          readOnly
            ? ["User ID / Username", "Spesialisasi", "SIP", "Poli & Jadwal Dinas"]
            : ["User ID / Username", "Spesialisasi", "SIP", "Poli & Jadwal Dinas", "Aksi"]
        }
        renderRow={(item, i) => {
              ...
              {!readOnly && (
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    <Button size="sm" variant="outline" onClick={() => handleAssignClick(item)} className="font-semibold text-xs text-blue-700 hover:text-blue-800 hover:bg-blue-50">
                      Assign Poli
                    </Button>
                    {item.poli_code && (
                      <Button
                        size="sm"
                        variant="ghost"
                        className="text-red-600 hover:text-red-700 hover:bg-red-50 text-xs flex items-center gap-1 cursor-pointer"
                        onClick={() => setDoctorToUnassign(item)}
                      >
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 border border-slate-100 animate-in fade-in zoom-in-95 duration-150">
            <div className="mb-4 border-b border-slate-100 pb-3">
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Dokter: <span className="font-semibold text-blue-700">{selectedDoctor?.username}</span>{" "}
                {selectedDoctor?.spesialisasi && `(${selectedDoctor.spesialisasi})`}
              </p>
            </div>
              ...
              <div className="p-3 bg-blue-50/70 border border-blue-200/80 rounded-lg flex items-start gap-2.5 text-xs text-blue-900">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-blue-700 mt-0.5 leading-relaxed">
                    Poliklinik hanya memiliki 1 dokter jaga per hari. Multi-dokter di poliklinik yang sama dibagi berdasarkan hari tugas yang berbeda.
                  </p>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide">
                    Hari Tugas Praktek (Senin – Jumat)
                  </label>
                  <button
                    type="button"
                    ...
                    {WORK_DAYS.map((day) => {
                      ...
                      if (isOccupied) {
                        return (
                          <div
                            key={day.id}
                            title={`Hari ${day.label} sudah ditugaskan kepada dr. ${occupant}`}
                ...
                {selectedPoli && WORK_DAYS.every((w) => occupiedDays[w.id]) && (
                  <div className="mt-2.5 p-2.5 bg-amber-50 border border-amber-200 rounded-lg text-[11px] text-amber-800 flex items-start gap-2">
                    ...
                    <span>
                      Semua hari kerja (Senin–Jumat) di poliklinik ini sudah terisi penuh oleh dokter lain. Silakan gunakan menu <Link to="/admin/master/jadwal-poliklinik" className="font-bold underline text-amber-900">Jadwal Poliklinik</Link> untuk menukar jadwal, atau lepas penugasan dokter yang ada.
                    </span>
                  </div>
                )}
              </div>
            ...
            <div className="flex items-center justify-between gap-3 mt-6 pt-3 border-t border-slate-100">
              {selectedDoctor?.poli_code ? (
                <Button
                  type="button"
                  variant="outline"
                  size="sm"
                  onClick={() => setDoctorToUnassign(selectedDoctor)}
      ...
      <ModernConfirmModal
        isOpen={!!doctorToUnassign}
        onClose={() => !isUnassigning && setDoctorToUnassign(null)}
        onConfirm={handleConfirmUnassign}
        isLoading={isUnassigning}
        title="Lepas Penugasan Dokter"
        description={
          <span>
            Apakah Anda yakin ingin melepas penugasan{" "}
            <strong className="text-slate-800 font-semibold">
              dr. {doctorToUnassign?.nama_lengkap || doctorToUnassign?.username}
            </strong>{" "}
            dari poliklinik?
          </span>
        }
        ...
        itemDetails={[
          {
            label: "Dokter",
            value: `dr. ${doctorToUnassign?.nama_lengkap || doctorToUnassign?.username}`,
          },
          {
            label: "Poliklinik",
            value: (
              <span className="text-blue-700 bg-blue-50 px-2 py-0.5 rounded font-bold">
                {doctorToUnassign?.poli_nama || `Poli ${doctorToUnassign?.poli_code}`}
              </span>
            ),
          },
          {
            ...
            value: (
              <span className="text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded font-bold">
                {formatSchedule(doctorToUnassign?.days_of_week)?.dayText || "-"}
              </span>
            ),
          },
        ]}
        note="Jadwal hari dinas dokter ini akan segera dikosongkan sehingga hari tersebut dapat ditugaskan ke dokter lain."
      />
    </>
  );
}
