...
const buildNavGroups = (poliLabel: string): NavGroup[] => [
  {
    title: null,
    items: [
      { name: "Dashboard", path: "/rawat-jalan/dokter", exact: true, icon: LayoutDashboard },
    ],
  },
  {
    title: poliLabel,
    items: [
      { name: "Antrean Pasien", path: "/rawat-jalan/dokter/antrean", icon: ListChecks },
    ],
  },
  {
    title: "Riwayat",
    items: [
      { name: "Riwayat Kunjungan", path: "/rawat-jalan/dokter/riwayat", icon: ClipboardPlus },
    ],
  },
];

export function DokterLayout() {
  const [isSidebarOpen, setIsSidebarOpen] = useState(true);
  const location = useLocation();
  const navigate = useNavigate();
  const { userId, role, poliName, poliCode, logout } = useAuth();

  const poliLabel = poliName || (poliCode ? `Poli (${poliCode})` : "Poliklinik");
  const navGroups = buildNavGroups(poliLabel);
  ...
  const currentPageName =
    navGroups
      .flatMap((g) => g.items)
      .find((item) => isActive(item.path, item.exact))?.name || "Dokter";
      ...
      >
        ...
        >
          {!isSidebarOpen ? (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="leading-none">
                <h1 className="font-bold text-base text-slate-800 tracking-tight">Rawat Jalan</h1>
          ...
          <Button
            ...
            size="icon"
            className={cn(
              "h-7 w-7 text-slate-400 hover:bg-slate-100 hover:text-slate-700 rounded-full",
              !isSidebarOpen
                ? "absolute -right-3.5 top-4 bg-white border border-slate-200 shadow-sm"
                : ""
            )}
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3 pl-2 pr-1 py-1 rounded-full border border-slate-200 bg-white shadow-sm hover:shadow-md transition-all">
              <div className="h-8 w-8 rounded-full bg-gradient-to-br from-emerald-500 to-emerald-400 flex items-center justify-center text-white font-bold text-sm uppercase shadow-inner">
                {role ? role.substring(0, 1).toUpperCase() : "D"}
              </div>
              <div className="text-right hidden md:block">
                <p className="text-sm font-bold text-slate-800 leading-tight">dr. {poliLabel}</p>
