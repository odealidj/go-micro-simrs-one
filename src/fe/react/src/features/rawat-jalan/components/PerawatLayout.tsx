...
const buildNavGroups = (poliLabel: string): NavGroup[] => [
  {
    title: null,
    items: [
      { name: "Dashboard", path: "/rawat-jalan/perawat", exact: true, icon: LayoutDashboard },
    ],
  },
  {
    title: poliLabel,
    items: [
      { name: "Antrean Pasien", path: "/rawat-jalan/perawat/antrean", icon: ListChecks },
      { name: "Input Triage", path: "/rawat-jalan/perawat/triage", icon: ClipboardEdit },
    ],
  },
];

export function PerawatLayout() {
  ...
  const currentPageName =
    navGroups
      .flatMap((g) => g.items)
      .find((item) => isActive(item.path, item.exact))?.name || "Perawat";
          ...
          {!isSidebarOpen ? (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="leading-none">
                ...
                <span className="text-[10px] font-semibold text-cyan-600 tracking-wider uppercase">
                  {poliLabel}
                </span>
              </div>
            </div>
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3 pl-2 pr-1 py-1 rounded-full border border-slate-200 bg-white shadow-sm hover:shadow-md transition-all">
              <div className="h-8 w-8 rounded-full bg-gradient-to-br from-cyan-500 to-cyan-400 flex items-center justify-center text-white font-bold text-sm uppercase shadow-inner">
                {role ? role.substring(0, 1).toUpperCase() : "P"}
              </div>
              <div className="text-right hidden md:block">
                <p className="text-sm font-bold text-slate-800 leading-tight">Perawat</p>
