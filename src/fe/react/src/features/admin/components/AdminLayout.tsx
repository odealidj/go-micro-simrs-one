  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      >
          ...
          {!isSidebarOpen ? (
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="leading-none">
                ...
                <span className="text-xs font-semibold text-blue-600 tracking-wider uppercase">SIMRS Admin</span>
              </div>
            </div>
          )}
        ...
        {isSidebarOpen && (
          <div className="px-4 pt-3 pb-1.5 flex items-center justify-between border-b border-slate-100 text-xs text-slate-500 font-medium">
            ...
            <button
              ...
              className="text-blue-600 hover:text-blue-700 hover:underline cursor-pointer font-semibold text-xs"
            >
        ...
        {/* Sidebar Navigation */}
        <div className="flex-1 py-3 overflow-y-auto px-3 space-y-2.5 custom-scrollbar">
          {navGroups.map((group) => {
              ...
              <div key={group.id} className="space-y-1">
                ...
                {isSidebarOpen ? (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg text-[13px] font-semibold tracking-normal transition-all cursor-pointer ${
                  ...
                  <div className="relative group flex justify-center py-1">
                    ...
                    <div className="absolute left-16 top-0 bg-white border border-slate-200 shadow-xl rounded-xl p-2.5 w-60 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150 z-50">
                      <div className="px-2.5 py-1.5 mb-1.5 border-b border-slate-100 font-bold text-sm text-slate-800 flex items-center justify-between">
                ...
                {isSidebarOpen && isExpanded && (
                  <nav className="ml-4 pl-3.5 border-l-2 border-slate-200/80 space-y-1 my-1.5 transition-all duration-200">
                    {group.items.map((item) => {
                      const isActive =
                      ...
                      return (
                        <Link
                          key={item.path}
                          to={item.path}
                          className={`flex items-center px-3 py-2 rounded-lg text-sm font-medium transition-colors group relative cursor-pointer ${
                            isActive
                              ? "bg-blue-50 text-blue-700 font-semibold border-l-2 border-blue-600 -ml-[16px] pl-[26px]"
                              : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"
                          }`}
                        >
                          <item.icon
                            className={`h-[18px] w-[18px] shrink-0 mr-3 ${
                              isActive
                                ? "text-blue-600"
